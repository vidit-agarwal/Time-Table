...
if (isset($_POST['add'])) {
   //connect to daatabase;
     ...
     $query = $mysqli->query("SELECT * FROM subject WHERE s_name ='$name' and s_sem='$sem' and s_id='$id' and s_branch='$branch' ");
     if(empty($name) OR empty ($id) OR empty($sem) or empty($branch) or empty($lab)  )
     {
     ...
     else
     {
      ...
      $insert =$mysqli->query("INSERT INTO subject (s_name, s_sem, s_id, s_branch,s_lab) VALUES ('$name','$sem','$id','$branch','$lab')");
      if($insert != TRUE)
      {
      	echo '<script type="text/javascript">
...
<div class="logo_name">
  ...
  <div class="body">
		...
		<div class="login">
				...
				Select Semester :<br> <select name ="sno">
<option value="1">1</option>
  ...
  <br>
				...
				Select Branch: <br> <select name ="branch">
