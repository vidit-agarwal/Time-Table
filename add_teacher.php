...
if (isset($_POST['add'])) {
   //connect to daatabase;
     ...
     if(empty($name) OR empty ($desig) OR empty($depart) OR empty($email)  OR empty($contact))
     {
     ...
     else
     {
      ...
      $insert =$mysqli->query("INSERT INTO teacher (t_name, t_dept, t_desig, t_email,  t_numb) VALUES ('$name','$depart','$desig','$email', '$contact')");
      if($insert != TRUE)
      {
      	echo '<script type="text/javascript">
